...
void flag_do_h(Z80_Register *param_reg, int value_left, int value_right, bool subtraction) {
	if (subtraction) {
		param_reg->flag_h = uint8_t((value_left - value_right) & 0xF0 ) < (value_left & 0xF0) ? 1 : 0;
	} else {
		param_reg->flag_h = ( ((value_left & 0xF) + (value_right & 0xF)) > 0xF) ? 1 : 0;
	}
}
...
void cpu_instruction_xor(Z80_Register *param_reg, uint8_t param_value) {
	...
	param_reg->flag_z = (param_reg->a == 0) ? 1 : 0;
	param_reg->flag_n = 0;
	param_reg->flag_h = 0;
	param_reg->flag_c = 0;
}
void cpu_instruction_cp(Z80_Register *param_reg, uint8_t param_value) {
	param_reg->flag_z = (param_reg->a == param_value) ? 1 : 0;
	param_reg->flag_n = 1;
	flag_do_h(param_reg, param_reg->a, param_value, true);
	param_reg->flag_c = (param_reg->a < param_value);
}
...
void prefix_cb(int opcode) {
	...
	switch(opcode) {
		...
		case 0x78: case 0x79: case 0x7A: case 0x7B: case 0x7C: case 0x7D: case 0x7F:
			...
			ptr_gb_reg->flag_z = ( ((*hold_ptr_u8) & reg_value_u8) == 0 ) ? 1 : 0;
			ptr_gb_reg->flag_n = 0;
			ptr_gb_reg->flag_h = 1;
			break;
		default:
	...
	while (!program_stop) {
		...
		switch (opcode) {
			...
			case 0x07:
				...
				ptr_gb_reg->flag_z = ( ptr_gb_reg->a == 0 ) ? 1 : 0;
				ptr_gb_reg->flag_h = 0;
				ptr_gb_reg->flag_n = 0;
				ptr_gb_reg->flag_c = hold_bit;
				break;
			...
			case 0x17:
				hold_bit = (ptr_gb_reg->a & 0x80) >> 7;
				ptr_gb_reg->a = (ptr_gb_reg->a << 1) + ptr_gb_reg->flag_c;
				ptr_gb_reg->flag_z = (ptr_gb_reg->a == 0) ? 1 : 0;
				ptr_gb_reg->flag_h = 0;
				ptr_gb_reg->flag_n = 0;
				ptr_gb_reg->flag_c = hold_bit;
			...
			case 0x1F:
				hold_bit = (ptr_gb_reg->a & 1);
				ptr_gb_reg->a = (ptr_gb_reg->flag_c << 7) + (ptr_gb_reg->a >> 1);
				ptr_gb_reg->flag_z = (ptr_gb_reg->a == 0) ? 1 : 0;
				ptr_gb_reg->flag_h = 0;
				ptr_gb_reg->flag_n = 0;
				ptr_gb_reg->flag_c = hold_bit;
			...
			case 0xF8:
				hold_byte = read_byte(gb_memory, pc + 1);
				ptr_gb_reg->flag_h = uint32_t((ptr_gb_reg->sp & 0x0FFF) + ((hold_byte) & 0x0FFF)) > 0x0FFF ? 1 : 0;
				ptr_gb_reg->flag_c = uint32_t( ptr_gb_reg->sp + (int8_t)(hold_byte) ) > 0xFFFF? 1 : 0;
				ptr_gb_reg->flag_z = 0;
				ptr_gb_reg->flag_n = 0;
			...
			case 0x28: // JP Z, signed d8 + pc
				...
				cpu_cc_jp_signed(ptr_gb_reg, ptr_gb_reg->flag_z, parse_opcode_val, int8_t(read_byte(gb_memory, pc + 1)));
				break;
			case 0x30: // JP NC, signed d8 + pc
			case 0x38: // JP C, signed d8 + pc
				parse_opcode_val = ((opcode & 0x08) >> 3);
				cpu_cc_jp_signed(ptr_gb_reg, ptr_gb_reg->flag_c, parse_opcode_val, int8_t(read_byte(gb_memory, pc + 1)));
				break;
			// RET Z FLAG
			case 0xC0: case 0xC8:
				parse_opcode_val = ((opcode & 0x08) >> 3);
			...
			case 0xC2: case 0xCA:
				...
				cpu_cc_jp_unsigned(ptr_gb_reg, ptr_gb_reg->flag_z, parse_opcode_val, read_short(gb_memory, pc + 1));
				break;
			// JP a16
			case 0xC3:
			...
			case 0xC4: case 0xCC:
				parse_opcode_val = (0x08 & opcode) >> 3;
				cpu_cc_call(ptr_gb_reg, gb_memory, ptr_gb_reg->flag_z, parse_opcode_val, pc);
				break;
			// CALL C FLAG
			case 0xD4: case 0xDC:
				parse_opcode_val = (0x08 & opcode) >> 3;
				cpu_cc_call(ptr_gb_reg, gb_memory, ptr_gb_reg->flag_c, parse_opcode_val, pc);
				break;
			// JP C FLAG, a16
			case 0xD2: case 0xDA:
				parse_opcode_val = (0x08 & opcode) >> 3;
				cpu_cc_jp_unsigned(ptr_gb_reg, ptr_gb_reg->flag_c, parse_opcode_val, read_short(gb_memory, pc + 1));
				break;
			// PUSH rr
			...
			case 0x09: case 0x19: case 0x29: case 0x39:
				...
				ptr_gb_reg->flag_n = 0;
				ptr_gb_reg->flag_h = uint32_t((ptr_gb_reg->hl & 0x0FFF) + ((*hold_ptr_u16) & 0x0FFF)) > 0x0FFF ? 1 : 0;
				ptr_gb_reg->flag_c = uint32_t( ptr_gb_reg->hl + (*hold_ptr_u16) ) > 0xFFFF? 1 : 0;
			...
			case 0x80: case 0x81: case 0x82: case 0x83: case 0x84: case 0x85: case 0x87:
				...
				ptr_gb_reg->flag_c = (uint16_t(ptr_gb_reg->a + hold_byte) > 0xFF) ? 1 : 0;
			...
			case 0x98: case 0x99: case 0x9A: case 0x9B: case 0x9C: case 0x9D: case 0x9F:
				...
				ptr_gb_reg->flag_c = (ptr_gb_reg->a < hold_byte) ? 0 : 1;
				ptr_gb_reg->a -= hold_byte;
				ptr_gb_reg->flag_z = (ptr_gb_reg->a == 0) ? 1 : 0;
				ptr_gb_reg->flag_n = 1;
				break;
			// SBC A, (HL)
			case 0x9E:
				hold_byte = read_byte(gb_memory, ptr_gb_reg->hl) + ptr_gb_reg->flag_c;
				flag_do_h(ptr_gb_reg, ptr_gb_reg->a, hold_byte, true);
				ptr_gb_reg->flag_c = (ptr_gb_reg->a < hold_byte) ? 0 : 1;
				ptr_gb_reg->a -= hold_byte;
				ptr_gb_reg->flag_z = (ptr_gb_reg->a == 0) ? 1 : 0;
			...
			// DAA
			case 0x27:
				if ((ptr_gb_reg->a & 0x0F) > 0x09 || ptr_gb_reg->flag_h) {
					ptr_gb_reg->a += 0x06;
				}
				if ( ((ptr_gb_reg->a & 0xF0) >> 4) > 0x09 || ptr_gb_reg->flag_c) {
					ptr_gb_reg->a += 0x60;
					ptr_gb_reg->flag_c = 1;
				} else {
					ptr_gb_reg->flag_c = 0;
